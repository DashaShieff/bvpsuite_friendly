function [coeff,x1,valx1,x1tau,valx1tau,polynomials,sol_infinite,tau_infinite,solx1_infinite,x1_infinite,lambda,eigenfunction]=run(bvpfile,zeichnung,x1,start,bvpopt,ausgabe,praediktor,left,right)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% RUN calls the function EQUATIONS which solves the collocation
% problem; also the output of the calculations such as a graphical
% presentation of the solution is set up within this function
% 
% FUNCTION CALL:
%[coeff,x1,valx1,x1tau,valx1tau,polynomials,sol_infinite,tau_infinite]=run(bvpfile,zeichnung,x1,start,bvpopt,ausgabe,praediktor)
% INPUTS:  bvpfile      ... automatically generated m-file,stores data from
%                       GUI which determines the problem; 
%          zeichnung    ...  boolean; determines if there should be a
%                            graphical presentation of the solution
%          x1           ...  mesh of initial profile
%          start        ...  coefficients of start profile in Runge-Kutta basis 
%          bvpopt       ...  parameters for calculations; stored in a
%                            .mat-file
%          ausgabe      ...  ???
...
%                            praediktor(3,1) gemacht then the 
%                            sum(ordnung)+1  equation is replaced by p1=praediktor(3,1) 
% 
% OUTPUTS: coeff        ...  coefficients of solutions polynomials in
%                            Runge-Kutta basis
%          x1           ...  mesh;               
%          valx1        ...  solution values on mesh
%          x1tau        ...  grid = mesh including collocation points
%          valx1tau     ...  solution values on grid
%          polynomials  ...  coefficients of solution polynomials in
%                            monomial basis
%          sol_infinite ...  solution values on truncated interval if
%                            problem is posed on a semi-finite interval
%          tau_infinite ...  grid(=mesh incl. coll.points) on large, but
%                            truncated interval
%          lambda       ...  calculated eigenvalue
%          eigenfunction...  calculated eigenfunction components
% AUTHOR: gkitzhofer
% ADAPTED BY: csimon
% DATE: ??
...
%Falls zu wenig input Argumente, setze die entsprechenden restlichen auf
%leer:
solx1_infinite=[];
x1_infinite=[];
sol_infinite=[];
...
eigenfunction=[];
 ...
 if (nargin<7) praediktor=[]; if (nargin<6) ausgabe=1; if (nargin<5) bvpopt=[]; if (nargin<4) start=[];if(nargin<3) x1=[];if(nargin<2) zeichnung=false;end;end;end;end;end;end;
...
if(length(x1)==0) 
    ...
    %Wenn kein spezielles Gitter (mesh) eingegeben wurden, dann ...
    x1=feval(bvpfile,'x1');  %nimm das aus dem bvpfile
    if(length(start)==0)     %Wenn in diesem Fall kein spezieller Startwert eingegeben wurde, dann ...
...
else                        %Wenn ein Gitter (mesh) eingegeben wurden, dann ...
    if(length(start)==0)    %Setze den Startvektor (unabhängig vom Eintrag im bvpfile) auf ...
       
        x0=ones(((length(x1))-1)*(feval(bvpfile,'n')*feval(bvpfile,'m')+sum(feval(bvpfile,'ordnung')))+feval(bvpfile,'parameter'),1);
        
    else
        x0=start;            %es wurde im Argument start anders angegeben.
    end
end

%Kompatibel mit Version 5


% Standardoptionen
if (length(bvpopt)==0)
    default_zf_opt = optimset('Display','off','MaxIter',90000000,'MaxFunEvals',90000000);
    defaultopt = sbvpset('AbsTol',1e-10,'Basis','RungeKutta','CheckJac',0,'ColPts','Equidistant','Degree',4,'DegreeSelect','auto','Display',1,'fVectorized',0,'IntMaxMinRatio',10,'JacVectorized',0,'MaxMeshPts',10000,'OutputFcn','','OutputSel',[],'OutputTrace',1,'RelTol',1e-10,'ZfOpt',default_zf_opt);
    bvpopt = sbvpset;
    bvpopt.ZfOpt = optimset(defaultopt.ZfOpt,bvpopt.ZfOpt);
    bvpopt = sbvpset(defaultopt, bvpopt);
    bvpopt.Log = 0;
    bvpopt.TRM = 0;
    bvpopt.errorestimate=0;
    bvpopt.Private.UpdateJacFactor = 0.5;
    bvpopt.Private.LambdaMin = 0.001;
    bvpopt.Private.SwitchToFFNFactor = 0.5;
end
bvpopt.praediktor=praediktor;

parameters = [];  % list of further parameters
% Ende Standardoptionen
parameters(1)=ausgabe;
ordnung=feval(bvpfile,'ordnung');
rho=feval(bvpfile,'rho');
standard=feval(bvpfile,'standard');
try
    linear=feval(bvpfile,'linear');
    if linear
        if ((parameters(1)==1) || (parameters(1)==2))
            fprintf('Solve problem by linear solver ...\n');
        end
    end
catch
    fprintf('Warning: Your bvpfile is not up to date, the check for linearity failed!\n');
    fprintf('Your problem is assumed to be nonlinear!\n');
    fprintf('Open it in bvpsuite and press ''Save''!\n');
    linear=0;
end
if (standard)
    if (rho(1)==1)
...
if ~linear
    ...
    [coeff,logstruct,itcount,fcount] = solve_nonlinear_sys(@F,@DF,@FDF,x0,bvpfile,x1,bvpopt,parameters,psival,psi);
    polynomials = logstruct.tol_factor;
    
else
    praediktor=bvpopt.praediktor;
...
% condest(outputDF)
%Ende Einschub
if length(coeff)>feval(bvpfile,'parameter')
    if (parameters(1)==2)
    ...
    valx1tau=equations('valx1tau',coeff,bvpfile,x1,[],[],psival,psi);
    x1tau=equations('x1tau',coeff,bvpfile,x1,[],[],psival,psi);
    valx1=equations('valx1',coeff,bvpfile,x1,[],[],psival,psi);        
    
    else
        valx1tau=equations('valx1tau',coeff,bvpfile,x1,[],[],psival,psi);
        x1tau=equations('x1tau',coeff,bvpfile,x1,[],[],psival,psi);
        %  DASHA EDIT DO HALF WAY PLOTS
        %filename = strcat('halfwayfCount',num2str(fcount),'itCount',num2str(itcount))
        %save(filename,'x1tau','valx1tau');
    ...
    if feval(bvpfile,'Infinite')==1
        ...
        if ep == 0
 	    ...
 	    for i=1:n/2
                ...
                [solx1_infinite(i,:),x1_infinite]= backtransf(valx1(i,2:end),valx1(i+n/2,2:end),x1(2:end),ep);
            end
            ...
            for i=1:n
                ...
                [solx1_infinite(i,:),x1_infinite]= backtransf([],valx1(i,2:end),x1(2:end),ep);
        ...
        if feval(bvpfile,'EVP')
            ...
            lambda=sol_infinite(end-1,1);
            eigenfunction=sol_infinite(1:end-2,:);

            if zeichnung==1
                plot_results(tau_infinite,[eigenfunction;sol_infinite(end-1,:)],size(eigenfunction,1),1,1,left,right);
            end
        ...
        if feval(bvpfile,'EVP')
            ...
            lambda=valx1tau(end-1,1);
            eigenfunction=valx1tau(1:end-2,:);
...
function outputF = F(bvpfile,a,x1,bvpopt,parameters,psival,psi)
praediktor=bvpopt.praediktor;
outputF = equations('F',a,bvpfile,x1,[],praediktor,psival,psi);
if ((parameters(1)==1) || (parameters(1)==2))
    fprintf('||F(x)|| = %8e\n',norm(outputF));
end
if parameters(1)==2
    nachricht=strrep(strcat('|| F(x) ||   =???',num2str(norm(outputF))),'???','   ');
    msgbox(nachricht,'Info','modal');
end

function outputDF = DF(bvpfile,a,x1,bvpopt,parameters,psival,psi)
praediktor=bvpopt.praediktor;
outputDF=equations('DF',a,bvpfile,x1,[],praediktor,psival,psi);

function [outputF,outputDF] = FDF(x0,bvpfile,x1,bvpopt,parameters,psival,psi)
praediktor=bvpopt.praediktor;
outputF = equations('F',x0,bvpfile,x1,[],praediktor,psival,psi);
outputDF = equations('DF',x0,bvpfile,x1,[],praediktor,psival,psi);
...
%Achtung! n in psi(:,:,n) ist nicht mehr - im Gegensatz zu
%früheren Versionen - die Ableitung von Psi, sondern die Anzahl, wie oft die
%Polynome in der Lagrangebasis integriert wurden. Für n. Ordnung enspricht
%also psi(:,:,n) dem nicht abgeleiteten Psi.
